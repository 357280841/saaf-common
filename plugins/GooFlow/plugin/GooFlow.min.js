...
GooFlow.prototype.mark3 = function (id, type, mark) {//add by liujun on 2020-06-01
    if (type == "node") {
        if (!this.$nodeData[id]) {
            return
        }
        ...
        this.$nodeData[id].marked = mark || false;
        if (mark) {
            this.$nodeDom[id].addClass("item_mark3")
        } else {
            this.$nodeDom[id].removeClass("item_mark3")
        }
    } else {
        if (type == "line") {
            if (!this.$lineData[id]) {
            ...
            if (GooFlow.prototype.useSVG != "") {
                ...
                jq[0].childNodes[1].setAttribute("stroke", "#9aa9e2");
                jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow4)")
            } else {
                if (mark) {
                    jq[0].strokeColor = "#9aa9e2"
                } else {
                    jq[0].strokeColor = "#ccc"
                }
            }
...
GooFlow.prototype.addNode = function (id, json) {
    ...
    // var mark = json.mark ? " item_mark" : (json.customMarked ? " item_mark1" : "");//update by liujun on 2020-04-27
    var mark = json.mark ? " item_mark" : ((json.customMarked ? " item_mark1" : (json.preMarked ? " item_mark3" : "")));
    if (json.type != "start" && json.type != "end") {
        ...
        if (json.users) {
            for (var i =0; i < json.users.length; i++) {
                nodeNames.push(json.users[i].userFullName);
            }
        }

        var nodename =nodeNames.length > 0 ? nodeNames.splice('，') : '';
        var nodeTitle = nodename;//add by liujun on 2020-06-01
        //remove by liujun on 2020-06-01
        // var nodeTitle = '';
        //
...
GooFlow.prototype.addLine = function (id, json) {
    ...
    if (GooFlow.prototype.useSVG == "") {
        ...
        // 获取坐标开始与结束
        var from_xy = $(this.$lineDom[id]).attr('from').split(',')
        var to_xy = $(this.$lineDom[id]).attr('to').split(',')
        // 根据坐标差计算宽度
        var width = to_xy[0] - from_xy[0]
...
function processImageDraw(certificate, locationId, processInstanceId, processDefinitionKey, processDefinitionId,businessKey,preview) {


    // var destine = '//192.168.21.116';//测试环境
    var host =  localStorage.getItem('ausnutriaSystemServerAPI');
    if(!host) {
        alert('获取服务地址错误，请联系管理员！');
        return;
    }

    var _bpm_process_image_post_url = `${window.location.origin}/api/bpmServer/bpmProcessService/getGooflowProcessJSON`;
    // if (window.location.hostname == 'localhost') {
        // var _bpm_process_image_post_url = `http://60.167.163.119:9008/bpmProcessService/getGooflowProcessJSON`;//前端用
        // _bpm_process_image_post_url = "http://192.168.28.70:9008/bpmProcessService/getGooflowProcessJSON";//后端用
    // }
        ...
        success: function (res) {
            ...
            if (data.nodes) {
                for (var nodeId in data.nodes) {
                    ...
                    } else if (data.nodes[nodeId].preMarked) {//add by liujun on 2020-06-01
                        image.mark3(nodeId, "node", true);
                    }
                }
            }
            if (data.lines) {
                for (var lineId in data.lines) {
                    ...
                    } else if (data.lines[lineId].preMarked) {//add by liujun on 2020-06-01
                        image.mark3(lineId, "line", true);
                    }
                }
            }
...
function processImageDrawNew(certificate, locationId, params) {
    ...
    // if (window.location.hostname == 'localhost') {
        //   var _bpm_process_image_post_url = `http://60.167.163.119:9008/bpmProcessService/getGooflowProcessJSON`;//前端用
        // var _bpm_process_image_post_url = "http://192.168.28.70:9008/bpmProcessService/getGooflowProcessJSON";//后端用
    // }
...
//add by liujun on 2020-06-01
function processImageDrawPreview(certificate, locationId, params) {
    // var destine = '//192.168.21.116';//测试环境
    var host =  localStorage.getItem('ausnutriaSystemServerAPI');
    if(!host) {
        alert('获取服务地址错误，请联系管理员！');
    ...
    var _bpm_process_image_post_url = `${window.location.origin}/api/bpmServer/bpmProcessService/getPreviewGooflowJson`;
    // if (window.location.hostname == 'localhost') {
        //var _bpm_process_image_post_url = `http://60.167.163.119:9008/bpmProcessService/getGooflowProcessJSON`;//前端用
        // var _bpm_process_image_post_url = "http://localhost:9008/bpmProcessService/getPreviewGooflowJson";//后端用
    // }
    //$("body").append("<style>#" + locationId + "{opacity: 0}#" + locationId + " g{cursor: default!important;}</style>");
    var property = {
        haveHead: false,      //是否显示头部工具栏
            ...
            if (data.nodes) {
                for (var nodeId in data.nodes) {
                    if (data.nodes[nodeId].marked) {
                        ...
                        image.mark2(nodeId, "node", true);
                    } else if (data.nodes[nodeId].preMarked) {
                        image.mark3(nodeId, "node", true);
                    }
                }
            }
            if (data.lines) {
                for (var lineId in data.lines) {
                    if (data.lines[lineId].customMarked) {
                        ...
                        image.mark2(lineId, "line", true);
                    } else if (data.lines[lineId].preMarked) {
                        image.mark3(lineId, "line", true);
                    }
                }
            }
